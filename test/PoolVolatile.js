...
describe("PoolVolatile", async function () {
    ...
    beforeEach(async function () {
        ...
        usdtProxy = await MockAPI3Proxy.deploy(toWei('1.2'))
        await usdtProxy.deployed()
        usdtFeed = await API3FeedReader.deploy(usdtProxy.address)
        await usdtFeed.deployed()

        daiProxy = await MockAPI3Proxy.deploy(toWei('0.8'))
        await daiProxy.deployed()
        daiFeed = await API3FeedReader.deploy(daiProxy.address)
        await daiFeed.deployed()

        const PoolVolatile = await ethers.getContractFactory("PoolVolatile");
        pool = await upgrades.deployProxy(PoolVolatile, [zeroAddress, treasury, poolHelper.address], { initializer: 'initialize' });
        await pool.deployed();
    ...
    it("Test Swap 1:1 usdc-usdt", async function () {
        ...
        let finalAmount = 1 / 1.2 * swapAmount * (1 - calculatedSlippage)
    ...
    it("Test Swap 1:1 usdc-dai", async function () {
        ...
        let newToSlippage = getSlippage(0.9)
        let calculatedSlippage = ((newFromSlippage - oldFromSlippage) / (0.1)) + ((newToSlippage - oldToSlippage) / (0.1))
        let finalAmount = 1 / 0.8 * swapAmount * (1 - calculatedSlippage)
        let feeAmount = finalAmount * 0.0001
        let toAmount = finalAmount - feeAmount;
        let treasuryFees = feeAmount * 0
    ...
    it("Test Swap 1:1 dai-usdt", async function () {
        ...
        let newToSlippage = getSlippage(0.9)
        let calculatedSlippage = ((newFromSlippage - oldFromSlippage) / (0.1)) + ((newToSlippage - oldToSlippage) / (0.1))
        let finalAmount = 0.8 / 1.2 * swapAmount * (1 - calculatedSlippage)
        let feeAmount = finalAmount * 0.0001
        let toAmount = finalAmount - feeAmount;
        let treasuryFees = feeAmount * 0
    ...
    it("Test Withdraw other", async function () {
        ...
        await expect(pool.withdrawOther(usdc.address, dai.address, deployer, toMwei('1500'), 0, 2652351324))
            .to.be.revertedWith("LR low");
        await expect(pool.withdrawOther(dai.address, usdt.address, deployer, toWei('6000'), 0, 2652351324))
            .to.be.revertedWith("From LR higher");
        await pool.swap(usdc.address, dai.address, deployer, toMwei('3000'), 0, 2652351324);
        await expect(pool.withdrawOther(usdc.address, usdt.address, deployer, toMwei('11500'), 0, 2652351324))
            .to.be.revertedWith("From LR higher");
        ...
        expect(parseFloat(fromMwei(data1.otherAmount))).to.be.closeTo(1 / 1.2 * 1000, 1)
        ...
        expect(parseFloat(fromMwei(data2.otherAmount))).to.be.closeTo(0.8 / 1.2 * 1000, 1)
        ...
        withdrawData = await pool.getWithdrawAmount(lpusdc.address, toMwei('100'), false)
        ...
        await pool.withdrawOther(usdc.address, dai.address, deployer, toMwei('100'), 0, 2652351324)
        ...
        expect(oldLpUsdcBalance - newLpUsdcBalance).to.be.closeTo(100, 0.1)
    ...
    it("Test One Tap", async function () {
        ...
        let swapFinalAmount = 1.2 / 1 * withdrawnAmount * (1 - calculatedSlippage)
        ...
        expect(parseFloat(fromMwei(data.withdrawFees))).to.be.closeTo(withdrawFees, withdrawFees / 100)
        expect(parseFloat(fromMwei(data.depositLpAmount))).to.be.closeTo((toAmount - depositFees), (toAmount - depositFees) / 1000)
        expect(parseFloat(fromMwei(data.depositFees))).to.be.closeTo(depositFees, depositFees / 50)
    ...
    it("Test One Tap Limit", async function () {
        ...
        await lpusdc.setLiabilityLimit(toMwei('100011'))
